...
class Main:
    ...
    if IgnitionRad > 0:
        ...
        for i in Ignitions.values():
            ...
            if IgnitionRad == 1:
                IgnitionSets[auxIg] = set([a[0] for a in AdjCells[i-1].values() if a != None])
                IgnitionSets[auxIg].add(i)
                print("Ignition sets:",IgnitionSets)
                auxIg += 1
            ...
            if IgnitionRad > 1:
                ...
                IgnitionSets[auxIg] = auxSet
                print("Ignition set radious ge 2:", IgnitionSets[auxIg])
    ...
    path_messages = '/Users/jaime/Documents/GitHub/Net_Results/Sub2/Messages'
    G = []
    nsim = 100
    if heuristic == 6:
        ...
        # fpv_list =dict(zip(nodos, [[] for i in range(NCells)]))

        for k in range(1, nsim + 1):
            # path_messages = '/Users/jaime/Documents/GitHub/Net_Results/Sub2_381/Messages/'
            H = nx.read_edgelist(path=path_messages + '/MessagesFile' + str(k) + '.txt',
                                 # H = nx.read_edgelist(path=path_messages+'Messages'+str(k)+'/MessagesFile.txt',
            ...
            for i in H.nodes:
                ...
                H.node[i]['fpv'] = sum([H.node[j]['profit'] for j in Arbol[i].nodes])
                G.node[i]['fpv'] = G.node[i]['fpv'] + H.node[i]['fpv']
    if heuristic == 7:

        nodos = range(1, NCells + 1)

        # Construimos un objeto MultiDiGraph (grafo dirigido)
        G = nx.MultiDiGraph()
        # Agregamos nodos desde lista "nodos"
        G.add_nodes_from(nodos)
        ...
        for k in range(1, nsim + 1):
            ...
            G.add_weighted_edges_from(H.edges(data='time'), weight='time')
    ...
    if heuristic == 8:
        ...
        for k in range(1, nsim + 1):
            ...
            G.add_weighted_edges_from(H.edges(data='time'), weight='time')
            for i in H.nodes:
                G.node[i]['freq_burn'] = G.node[i]['freq_burn'] + 1



    AverageUtility = []  # Guarda la utilidad promedio en una simulacion
    AverageBurntCells = []  # Guarda el % de celdas quemadas en una simulacion
    AverageAvailCells = []  # Guarda el % de celdas disponibles en una simulacion
    ...
    while Sim <= TotalSims:
        ...
        if Sim != 1:
            if WeatherOpt == "random":
                ...
                Ignitions, Weather_Obj, Plotter, DF = Init(Ignitions, WeatherOpt, plottrue, OutFolder, DF,
                                                           args, verbose, nooutput)
                #DF = Init(Ignitions, WeatherOpt, plottrue, OutFolder, DF, args, verbose, nooutput)[3]
        ...
        StatPath = os.path.join(OutFolder, "Statistics")
        ...
        if WeatherOpt == "rows" or WeatherOpt == "random":
                            ...
                            for bc in GotMsg_Set:
                                if bc not in BurntCells_Set:
                                    if (bc - 1) not in Cells_Obj.keys():
                                        Cells_Obj[bc - 1] = CellsFBP.Cells((bc), AreaCells, CoordCells[bc - 1],
                                                                           AgeCells,
                                                                           FTypeCells[bc - 1],
                                                                           coef_ptr[
                                                                               FTypes2[str.lower(GForestType[bc - 1])]],
                                    ...
                                    if Cells_Obj[bc - 1].FType != 0:
                                        Check_Burnt = Cells_Obj[bc - 1].get_burned(Fire_Period[Year - 1],
                                                                                   NMessages[bc - 1],
        ...
        if nooutput == False:
            print("\n----------------------------- Results -----------------------------")
            AverageUtility.append(sum(TotalUtility) / TotalYears)
            AverageBurntCells.append(np.round(len(BurntCells_Set) / NCells * 100.0, 3))
            AverageAvailCells.append(np.round(len(AvailCells_Set) / NCells * 100.0, 3))
            # General results
            ...
            print("Total Harvested Cells: ", len(HarvestCells_Set), "- % of the Forest: ",
                  np.round(len(HarvestCells_Set) / NCells * 100.0, 3), "%")
            '''
        ...
        if exceltrue == True and SaveMem != True:
            ...
            Output_Grid.ExcelOutput(Cells_Obj, Sim, OutFolder, heuristic, TotalSims, TotalYears,
                                    NCells, BurntCells_Set, AvailCells_Set, HarvestCells_Set, spotting)
        ...
        if OutMessages == True:
            ...
            file = open(MessagesPath + "/MessagesFile"+str(Sim)+".txt", "w")
    ...
    # print "Average Utility: ", AverageUtility
    # print "Average Burnt Cells:", AverageBurntCells
